...
export default async function main() {
    Util.loadFragment('nav', 'nav-fixed-1col2row.html').then(() => {
        Bootstrap.initDropdown(['languageDropdown', 'languageDropdownMobile']);
        Bootstrap.initCollapse('navbarToggler');
    });
    Util.loadFragment('footer', 'footer-classic.html').then(() => {
...
${message}`;
                ...
                const mailtoLink = `mailto:manual20151276@gmail.com?subject=Contact Form Submission from ${firstName} ${lastName}&body=${encodeURIComponent(emailBody)}`;
...
//   // Handle data-i18n-html (HTML content)
